...
scCommentMgr.Controller.prototype.createThread = function(pIndexNodeAnc) {
	...
	scServices.scCommentSvc
			.prepareRequest(
					"Create",
					this.fControlOriPathList.fList[pIndexNodeAnc].toString(),
					this.fParent.Model
...
scCommentMgr.Controller.prototype.updateComment = function(pRefUriNumber, pThreadNumber, pCommentNumber) {
	...
	scServices.scCommentSvc
			.prepareRequest(
					"Update",
					this.fParent.Model.getThreadRefUri(pRefUriNumber)
							+ "@"
	...
	if (pNode.attributes) {
		...
		if (vDataOrigin != null) {
			...
			if (vDataOriginString.charAt(0) == '@') {
				pString = vDataOriginString.substring(1, vDataOriginString
								.indexOf('?'))
						+ "@"
