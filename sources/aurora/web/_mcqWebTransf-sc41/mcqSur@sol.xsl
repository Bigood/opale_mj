							...
							<xsl:for-each select="sc:choices/sc:choice">
								...
								<li class="choiceList_ch ">
									<div class="choiceList_in ">
										<input type="radio" name="{$vIdObj}_input" id="{$vIdChoiceIn}" value="{$vIndex}" onclick="{$vIdObj}.setResp(this, {$vIndex});" aria-labelledby="{$vIdChoiceLbl}"/>
									</div>
									<div class="choiceList_sol " style="display:none;">
										<input type="radio" name="{$vIdObj}_sol" id="{$vIdChoiceSol}" value="" disabled="true" aria-labelledby="{$vIdChoiceLbl}"/>
									</div>
