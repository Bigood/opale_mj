...
<xsl:for-each select="sc:choices/sc:choice">scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N<xsl:value-of select="count(preceding-sibling::sc:choice)"/>_choiceExp"), "collapsed");
</xsl:for-each>scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_score"), "invisible");
scAssmntMgr.setToggleStatus(sc$("<xsl:value-of select="$vIdObj"/>_S_N75"), false);
var vRespSession = "";
...
this.initSeq("play");
	...
	onActionN75 : function(pButton){
		if(scAssmntMgr.isDisabled(pButton)) return;
		var vIsOn = scAssmntMgr.isToggleOn(pButton);
		if(vIsOn) {
scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_score"), "invisible");
this.setSolution(false); scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_globalExp"), "collapsed");
<xsl:for-each select="sc:choices/sc:choice">scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N<xsl:value-of select="count(preceding-sibling::sc:choice)"/>_choiceExp"), "collapsed"); </xsl:for-each>scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_responseTi"), "invisible");
var vForm=document.forms.<xsl:value-of select="$vIdObj"/>_form; for (var i=0; i &lt; vForm.<xsl:value-of select="$vIdObj"/>_input.length; i++) vForm.<xsl:value-of select="$vIdObj"/>_input[i].disabled = false; 
		} else {
scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_score"), "visible");
this.setSolution(true); scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_globalExp"), "visible");
<xsl:for-each select="sc:choices/sc:choice">scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N<xsl:value-of select="count(preceding-sibling::sc:choice)"/>_choiceExp"), "visible"); </xsl:for-each>scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_responseTi"), "visible");
var vForm=document.forms.<xsl:value-of select="$vIdObj"/>_form; for (var i=0; i &lt; vForm.<xsl:value-of select="$vIdObj"/>_input.length; i++) vForm.<xsl:value-of select="$vIdObj"/>_input[i].disabled = true; 
		}
		scAssmntMgr.setToggleStatus(pButton, ! vIsOn);
		scServices.assmntMgr.commit();
	},
	...
	refreshScoresFeedbacks : function(){
		...
		var vScoreNode = sc$("<xsl:value-of select="$vIdObj"/>_SNc0");
