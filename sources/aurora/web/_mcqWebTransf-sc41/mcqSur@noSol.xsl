...
scAssmntMgr.setToggleStatus(sc$("<xsl:value-of select="$vIdObj"/>_S_N50"), false);
var vRespSession = "";
...
this.initSeq("play");
	...
	onActionN50 : function(pButton){
		if(scAssmntMgr.isDisabled(pButton)) return;
		var vIsOn = scAssmntMgr.isToggleOn(pButton);
		if(vIsOn) {
scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_score"), "invisible");
this.setSolution(false); var vForm=document.forms.<xsl:value-of select="$vIdObj"/>_form; for (var i=0; i &lt; vForm.<xsl:value-of select="$vIdObj"/>_input.length; i++) vForm.<xsl:value-of select="$vIdObj"/>_input[i].disabled = false; 
		} else {
scAssmntMgr.setMode(sc$("<xsl:value-of select="$vIdObj"/>_N_score"), "visible");
this.setSolution(true); var vForm=document.forms.<xsl:value-of select="$vIdObj"/>_form; for (var i=0; i &lt; vForm.<xsl:value-of select="$vIdObj"/>_input.length; i++) vForm.<xsl:value-of select="$vIdObj"/>_input[i].disabled = true; 
		}
	...
	refreshScoresFeedbacks : function(){
		...
		var vScoreNode = sc$("<xsl:value-of select="$vIdObj"/>_SN8f");
