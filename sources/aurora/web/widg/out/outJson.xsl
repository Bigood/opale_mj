...
<xsl:stylesheet xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:sc="http://www.utc.fr/ics/scenari/v3/core" xmlns:redirect="com.scenari.xsldom.xalan.lib.Redirect" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://xml.apache.org/xslt/java" extension-element-prefixes="redirect" exclude-result-prefixes="sc java xhtml" version="1.0">
	...
	<xsl:template match="treeContent">{home:<xsl:apply-templates select="entry[1]"/>module:<xsl:apply-templates select="entry[2]" mode="hideRoot"/>,tools:<xsl:apply-templates select="entry[3]" mode="hideRoot"/>}<xsl:value-of select="execute(substring(computeStrDialog(string(entry[4]/@dialog), 'actUri:'),2))"/>
	</xsl:template>
	...
	<xsl:template match="entry" mode="hideRoot">
		<xsl:variable name="vUrl" select="substring(computeStrDialog(string(@dialog), 'actUri:'),2)"/>{"children":[<xsl:for-each select="entry">
			<xsl:apply-templates select="."/>
